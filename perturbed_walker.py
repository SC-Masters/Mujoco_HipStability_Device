...
for frame in range(toe_off, N):
    ...
    if preblend_steps <= i_shifted < preblend_steps + perturb_steps:
        ...
        if trunk_active:
            ...
            target_r = np.array([0.3, 0.3])  # RIGHT foot (stepping)
            ...
            for idx in [hip_add_r, hip_flex_r, hip_add_l, hip_flex_l, knee_l]:
                ...
                cancel_start = 0.5  # start halfway through perturbation
                cancel_gain = 1.8  # strength of correction (tune 1.0–3.0)
...
# SAVE TRAJECTORY
# -----------------------------
traj.save("ML_perturbation_trajectory.npz")
print("Saved ML_perturbation_trajectory.npz!!!")
